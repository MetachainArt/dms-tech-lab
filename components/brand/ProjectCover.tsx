...
function NetworkDiagram() {
  return <svg viewBox="0 0 600 260" className={styles.diagram} role="img" aria-label="광통신 교재의 OLT, FDF, 광 분배기, ONU 연결 경로를 요약한 도식">
...
function AxDiagram() {
  const steps = ["업무 지도", "역할 분리", "핸드오프", "90일 측정"];
  return <div className={styles.axDiagram} aria-label="AX 플레이북의 업무 지도, 역할 분리, 핸드오프, 90일 측정 과정 요약">
...
export default function ProjectCover({ work, index }: { work: ShowcaseWorkItem; index: number }) {
  ...
  const caption = network ? "광통신 교재 기반 개념도" : ax ? "AX 플레이북 내용 시각화" : material?.caption || work.title;
  return <figure className={`${styles.cover} ${network ? styles.network : ax ? styles.ax : styles[material?.treatment || "artwork"]}`}>
    ...
    {network ? <><p className={styles.networkTitle}>One signal.<br /><em>Many connections.</em></p><NetworkDiagram /></> : ax ? <AxDiagram /> : <div className={styles.mount}><Image src={material?.image || work.image} alt={caption} fill sizes="(max-width: 767px) 90vw, 45vw" /></div>}
