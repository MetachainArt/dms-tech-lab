...
export default function Writing({ posts }: { posts: MDXPost[] }) {
      ...
      <div className={styles.container}>
        <div className={styles.sectionHeader}>
          <div>
            ...
            <p className={styles.sectionDescription}>자동화, 도구, 감각, 운영 방식에 대해 실무에서 부딪히며 정리한 생각들입니다.</p>
          </div>
          <Link href="/blog" className={styles.textLink}>Read more <ArrowUpRight size={17} /></Link>
        </div>
        <div className={styles.writingList}>
          {posts.map((post, index) => (
            <Link key={post.slug} href={`/blog/${post.slug}`} className={styles.writingRow}>
              <div className={styles.writingMeta}><span>0{index + 1}</span><span>{String(post.frontMatter.date)}</span></div>
              <div><h3>{String(post.frontMatter.title)}</h3><p>{String(post.frontMatter.excerpt)}</p></div>
