...
export default function PromptCard({ prompt }: PromptCardProps) {
  ...
  if (isVisual && prompt.image) {
    return (
        <div 
            ...
            className="relative h-80 rounded-xl overflow-hidden cursor-pointer group border border-white/5 hover:border-white/20 transition-all shadow-lg"
        >
            {/* Background Image */}
            <Image 
                src={prompt.image} 
  ...
  return (
    <div className="h-full bg-[#1A1D24] rounded-xl border border-white/5 hover:border-neon-purple/30 transition-all duration-300 flex flex-col group overflow-hidden relative p-6">
            {/* Header */}
            <div className="flex justify-between items-start mb-4">
                <span className={`text-[10px] uppercase font-bold tracking-wider px-2 py-1 rounded bg-white/5 text-gray-400 border border-white/5`}>
                    {prompt.category}
                </span>
                <button 
                    onClick={handleCopy}
                    className="text-gray-400 hover:text-white transition-colors p-1.5 hover:bg-white/10 rounded-md"
                    title="Copy Prompt"
                >
            ...
            {/* Prompt Preview Area */}
            <div className="bg-black/30 rounded-lg p-3 mb-4 border border-white/5 font-mono text-xs text-gray-300 leading-relaxed max-h-32 overflow-y-auto custom-scrollbar relative">
                {prompt.promptContent}
            </div>
            ...
            <div className="mt-auto">
                 <div className="flex flex-wrap gap-2">
                    {prompt.tags.map(tag => (
                        <span key={tag} className="text-[10px] text-gray-500 flex items-center">
