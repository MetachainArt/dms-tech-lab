...
export default function SeriesHeader({ series, postCount }: SeriesHeaderProps) {
  const variant = colorVariants[series.color] || colorVariants.blue;
    ...
    <section className="px-6 pb-16 pt-8">
      <div className="mx-auto grid max-w-7xl gap-10 rounded-[36px] border border-paperfolio-line bg-paperfolio-surface p-7 shadow-[0_22px_80px_rgba(31,41,55,0.06)] lg:grid-cols-[320px_minmax(0,1fr)] lg:p-10">
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }}>
          <div className="relative overflow-hidden rounded-[28px] border border-paperfolio-line bg-white">
            <div className="relative aspect-[4/5]">
              <Image src={series.coverImage} alt={series.title} fill priority sizes="(max-width: 1024px) 100vw, 320px" className="object-cover" />
              <div className="absolute inset-0 bg-gradient-to-t from-paperfolio-text/70 via-paperfolio-text/10 to-transparent" />
              <div className="absolute inset-x-0 bottom-0 p-6 text-white">
                <p className="text-xs font-semibold uppercase tracking-[0.22em] text-white/70">{series.subtitle}</p>
                <h1 className="mt-3 font-playfair text-3xl leading-tight">{series.title}</h1>
              </div>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, delay: 0.08 }} className="space-y-6 lg:self-center">
          <span className={`inline-flex rounded-full px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] ${variant}`}>리도 인사이트</span>
          <div className="space-y-4">
            <h1 className="paperfolio-display text-paperfolio-text">{series.title}</h1>
            <p className="max-w-2xl text-lg leading-8 text-paperfolio-text-muted">{series.description}</p>
          </div>
          <div className="grid gap-4 grid-cols-2 sm:grid-cols-4">
            <div className="rounded-[24px] border border-paperfolio-line bg-white px-5 py-5">
              <p className="text-xs font-semibold uppercase tracking-[0.22em] text-paperfolio-text-muted">작성</p>
              <p className="mt-3 text-lg font-semibold text-paperfolio-text">Reedo</p>
            </div>
            <div className="rounded-[24px] border border-paperfolio-line bg-white px-5 py-5">
              <p className="text-xs font-semibold uppercase tracking-[0.22em] text-paperfolio-text-muted">편수</p>
              <p className="mt-3 text-lg font-semibold text-paperfolio-text">{postCount}편</p>
            </div>
            <div className="rounded-[24px] border border-paperfolio-line bg-white px-5 py-5">
              <p className="text-xs font-semibold uppercase tracking-[0.22em] text-paperfolio-text-muted">상태</p>
              <p className="mt-3 text-lg font-semibold text-paperfolio-accent-blue">연재 중</p>
            </div>
            <Link
              href={KAKAO_URL}
              target="_blank"
            ...
            >
              ...
              <p className="mt-3 text-lg font-semibold text-[#3c1e1e]">설치/문의</p>
            </Link>
          </div>
        </motion.div>
      </div>
