...
`,
      ...
      },
      {
        ...
        subtitle: "SELF-HOSTED SYNC GUIDE",
        description: "옵시디언을 더 잘 쓰기 위한 환경 구축, 자동화, 워크플로우 가이드 모음입니다. 첫 글은 매월 구독료 없이 본인 서버에 동기화 환경을 구축하는 방법을 단계별로 정리합니다.",
        coverImage: "/images/works/obsidian-cover.jpeg",
        color: "blue",
        tags: ["Obsidian", "CouchDB", "Caddy", "Self-hosted", "동기화", "SSL"],
        status: "진행 중",
        category: "지식 관리",
        steps: [
          {
            id: "step-1",
            title: "옵시디언 자체 동기화 서버 구축 가이드",
            excerpt: "CouchDB + Caddy + Self-hosted LiveSync로 매월 구독료 없이 본인 서버에 옵시디언 동기화 환경을 30분 만에 구축하는 방법. SSL 자동화부터 모바일 연결, 트러블슈팅까지 한 번에 정리합니다.",
            date: "2026. 04. 15",
            readTime: "30 min",
            content: `## 왜 자체 구축인가?
...
이 글에서 만드는 환경은 다르다. 본인의 PC나 NAS에 **CouchDB + Caddy** 조합으로 동기화 서버를 직접 띄우고, 옵시디언 커뮤니티 플러그인 **Self-hosted LiveSync**로 연결한다. 데이터는 외부 클라우드를 거치지 않고 본인 서버에만 머문다. 비용은 0원이다.

<Grid cols={3}>
  <Card icon="DollarSign" color="coral" title="Obsidian Sync 공식">
    월 $5~$10 · 1~10GB 제한 · 외부 서버 경유
  </Card>
  <Card icon="AlertTriangle" color="coral" title="Remotely Save">
    2025.07 유료화 예정 · 클라우드 의존
  </Card>
  <Card icon="Zap" color="teal" title="자체 구축 (이 가이드)">
...
| 기술 | 역할 | 비용 |
|---|---|---|
| **CouchDB** | NoSQL DB · 실시간 동기화 백엔드 | 무료 (오픈소스) |
| **Caddy** | 리버스 프록시 · SSL 자동 발급/갱신 | 무료 (오픈소스) |
| **Self-hosted LiveSync** | 옵시디언 커뮤니티 플러그인 | 무료 (오픈소스) |
| **Let's Encrypt** | SSL 인증서 | 무료 |

기존 서버를 활용한다면 **총 구축 비용은 0원**이다. CouchDB는 아이들 상태에서 RAM 50~100MB만 쓰고 CPU는 거의 0%다. NAS나 파일서버에 얹어도 부담이 없다.
...
| RAM | 512MB | 1GB 이상 |
| 디스크 | 1GB | 10GB 이상 |
| OS | Windows 10+ | Windows 10/11, Ubuntu 22.04+ |
...
> ❌ **PowerShell curl 함정**: PowerShell에서 \`curl\`은 \`Invoke-WebRequest\`의 별칭이다. 반드시 \`curl.exe\`로 실행해야 한다. 그렇지 않으면 "매개 변수 이름 'X'과(와) 일치하는 매개 변수를 찾을 수 없습니다" 오류가 난다.

> ⚠️ **비밀번호 특수문자**: \`!\`, \`)\`, \`@\` 같은 특수문자가 포함된 경우 작은따옴표로 감싼다.
>
...
|---|---|---|---|
| **80** | 서버 내부 IP | 80 | Let's Encrypt 인증서 발급/갱신 |
| **443** | 서버 내부 IP | 443 | HTTPS 동기화 트래픽 |
...
이 응답이 나오면 **DNS + SSL + 리버스 프록시**가 모두 정상이라는 뜻이다. 여기까지 왔다면 서버 쪽 작업은 사실상 끝난 것이다.
...
1. **"I am setting this up for the first time"** 선택
2. **"Yes, I want to set up a new synchronisation"** 클릭
3. **"Enter the server information manually"** 선택
4. E2E 암호화: 체크 + passphrase 입력 (**기억 필수**)
5. 서버 타입: **"CouchDB"** 선택
...
1. **"Detect and Fix CouchDB Issues"** → 표시되는 모든 Fix 버튼 클릭
2. **"All checks passed successfully!"** 확인
3. **"Test Settings and Continue"** 클릭
4. **"Restart and Initialise Server"** 클릭
5. 확인 체크박스 3개를 모두 체크 → **"I Understand, Overwrite Server"**
6. **"Replication completed"** 메시지 확인
...
이제 두 번째, 세 번째 기기를 같은 동기화 환경에 추가한다. 핵심은 **빈 볼트로 시작**해서 서버에 있는 데이터를 끌어오는 흐름이다.

### 설정 순서

1. 새 기기에서 옵시디언 실행 → **빈 볼트 새로 생성**
2. Self-hosted LiveSync 플러그인 설치 / 활성화
3. **"I am adding a device to an existing synchronisation setup"** 선택
4. **"Enter the server information manually"** 선택
5. E2E passphrase: **첫 번째 기기와 정확히 동일하게 입력**
6. 서버 정보: 첫 번째 기기와 동일하게 입력
7. Vault 상태: **"This Vault is empty..."** 선택
8. **"Reset and Resume Synchronisation"** 클릭
...
PowerShell 창에서 \`caddy.exe run\`으로 띄우면 그 창을 닫는 순간 Caddy가 종료되어 동기화가 끊긴다. Windows 서비스로 등록해서 백그라운드에서 항상 실행되도록 만든다.
...
**해결**: \`curl\` → \`curl.exe\`로 변경. PowerShell의 \`curl\`은 \`Invoke-WebRequest\`의 별칭이라 리눅스 curl과 문법이 완전히 다르다.
...
이 가이드의 핵심은 **세 가지 오픈소스를 조립**하는 것이다. CouchDB(저장), Caddy(SSL·프록시), Self-hosted LiveSync(클라이언트). 각각은 단순하지만 조합하면 상용 서비스급의 동기화 환경이 된다.
