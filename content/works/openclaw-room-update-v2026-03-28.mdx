...
| 항목 | 내용 |
|------|------|
| Memory 플러그인 | 메모리 압축(compaction) 시 flush 로직을 memory-core 플러그인이 직접 관리하도록 이동. 더 일관된 메모리 관리 |
| Plugins/runtime의 heartbeat | 플러그인에서 `runHeartbeatOnce`를 직접 호출해 특정 대상에게 하트비트 1회 전송 가능 |
| Agents/compaction | 압축 후 AGENTS 파일 갱신이 더 안정적으로 유지됨. 취소 이유가 더 구체적으로 표시됨 |
| TTS 설정 마이그레이션 | 구형 TTS API 키 형식을 자동 마이그레이션. `tts.<provider>` 형식 레거시 지원 종료 |
| Tavily 요청 헤더 | `X-Client-Source: openclaw` 헤더 추가. Tavily가 오픈클로발 트래픽을 구분할 수 있게 됨 |
| Doctor / 설정 마이그레이션 | 오래된 플러그인 경로를 자동으로 새 경로로 재작성 |
